...
def non_imaging(deg = 45):
    with open("imaging_non_%d.csv"%deg, "w", encoding="utf-8") as g:
...
def LP_imaging(deg = 45, flatisout = True, LP = 40):
    with open("imaging_LP%d_%d_%d.csv"%(LP,deg,flatisout), "w", encoding="utf_8") as g:
        print("方位角in0,方位角in1,仰角in,入射角ASKAin,仰角ASKAin,入射角ASKAout,仰角ASKAout,方位角out0,方位角out1,仰角out", file=g)
        for theta_in0 in range(-89,90):
...
def LPV_imaging(deg = 45, flatisout = True, LPV = 90):
    with open("imaging_LPV%d_%d_%d.csv"%(LPV,deg,flatisout), "w", encoding="utf_8") as g:
        print("方位角in0,方位角in1,仰角in,入射角ASKAin,仰角ASKAin,入射角ASKAout,仰角ASKAout,方位角out0,方位角out1,仰角out", file=g)
        for theta_in0 in range(-89,90):
...
def main(deg = 45, flatisout = True, LP = 40, LPV = 90):
    non_imaging(deg=deg)
    LP_imaging(deg=deg,flatisout=flatisout,LP=LP)
    LPV_imaging(deg=deg,flatisout=flatisout,LPV=LPV)
