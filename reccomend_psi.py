...
class psi():
    ...
    def test(self):
        r = psi.__idealver(45, n=1.51)
        for i in r:
            assert(-78<=i[0]<=-76)
            assert(-14<=i[1]<=-12)

    @staticmethod
    def __LP_minus(deg = 45, LP = 40):
        assert(0<=deg<=180)
        ret = []
        if -65+deg <= 90:
            out_mm = -65+deg
    ...
    def plot_all(self,LP=40):
        ...
        plt.xlabel("RT Plateとプリズムシートの成す角 [deg]", fontname="MS Gothic", fontsize = 14)
        # plt.ylabel("プリズムシート入射角（RT Plate基準） [deg]", fontname="MS Gothic")
        plt.ylabel("鑑賞方向（横角）[deg]", fontname="MS Gothic", fontsize = 14)
        plt.xlim(0,90)
        plt.ylim(-52,22)
        plt.show()
